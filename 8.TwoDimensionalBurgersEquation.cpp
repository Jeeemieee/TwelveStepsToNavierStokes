...
int main() {
    ...
    std::string fileName{"8_BurgersEquation2DData.txt"};
    ...
    for (int t{0};t<nt;t++)
    {
        ...
        for (int j{1};j<y.size()-1;j++)
        {
            ...
            {
                u[j][i] = un[j][i] - un[j][i]*dt*pow(dx,-1)*(un[j][i]-un[j][i-1]) - un[j][i]*dt*pow(dy,-1)*(un[j][i]-un[j-1][i]) + nu*dt*pow(dx,-2)*(un[j][i+1] - 2*un[j][i] + un[j][i-1]) + nu*dt*pow(dy,-2)*(un[j+1][i] - 2*un[j][i] + un[j-1][i]);
            }
        }
